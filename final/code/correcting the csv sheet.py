...
df = pd.read_csv('/Users/abhijayjain/Desktop/internship/Indira Securities/F&O Prediction/trade_details.csv')
...
new_df['Date'] = df['Date']
new_df['EnTime'] = df['Entry Time']
new_df['BPrice'] = df['Entry Price']
new_df['ExTime'] = df['Exit Time']
new_df['SPrice'] = df['Exit Price']
# new_df['Target'] = new_df['BPrice'] + (percentage / 100) * new_df['BPrice']
new_df['p/l'] = df['Profit/Loss']

    # Save the new DataFrame to a separate CSV file
new_df.to_csv('TRADE_DETAILS.CSV', index=False)
